...
struct ConfidenceScorer {
    ...
    private func calculateCompleteness(_ text: String, matchedPattern: OCRParsingEngine.CityPattern?) -> Double {
        ...
        if targetRange.contains(textLength) {
            return 1.0
        } else if abs(textLength - targetRange.lowerBound) <= 2 {
            return 0.7
        } else {
            return 0.4
        }
...
extension ConfidenceScorer {
    ...
    static func meetsAutoAcceptThreshold(_ confidence: Double) -> Bool {
        confidence >= ConfidenceLevel.high.threshold
    }
    
    /// Check if result requires review
    static func requiresReview(_ confidence: Double) -> Bool {
        confidence < ConfidenceLevel.high.threshold
    }
    
    /// Get user-friendly confidence message
    static func confidenceMessage(for level: ConfidenceLevel) -> String {
        switch level {
        case .high:
...
extension ConfidenceScorer {
    ...
    func shouldUseFallback(_ result: ScoreResult) -> Bool {
        return result.level == .low || result.recommendation == .reject
    }
    
    /// Suggest fallback preprocessing options
    func suggestFallbackOptions(_ result: ScoreResult) -> OCRPreprocessor.Options {
