...
struct OCRPreprocessor {
    ...
    struct Options {
        ...
        var correctPerspective: Bool = true
        var binarize: Bool = false
    ...
    func preprocess(_ uiImage: UIImage, options: Options = Options()) async throws -> UIImage {
    ...
    func adaptiveBinarize(_ uiImage: UIImage) async throws -> UIImage {
        guard let ciImage = CIImage(image: uiImage) else {
            throw PreprocessingError.invalidImage
        }
...
extension OCRPreprocessor {
    ...
    static func crop(_ uiImage: UIImage, to rect: CGRect) -> UIImage? {
    ...
    static func grayscale(_ ciImage: CIImage) -> CIImage? {
